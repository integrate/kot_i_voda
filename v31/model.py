...
def make_sun():
    global show_sun, drop_speed, drop_time, cloud_speed, water_size, drop_count
    if show_sun:
        return

    if drop_count<drops_on_level*2:
        return

    drop_count-=drops_on_level*2

    show_sun = True
    pygame.time.set_timer(sun_timer_id, 3000, 1)

    drop_speed -= 1
    if drop_speed<3:
        drop_speed=3

    drop_time=drop_time*1.1
    if drop_time>3000:
        drop_time=3000
...
show_sun = False
sun_timer_id = pygame.event.custom_type()
